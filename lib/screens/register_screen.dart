...
class _RegisterScreenState extends State<RegisterScreen> {
  ...
  // 5. Dropdown States (Matching ACTUAL Backend ENUMs)
  ...
  Future<void> _register() async {
    if (_nameController.text.trim().isEmpty || 
        _wabaIdController.text.trim().isEmpty || 
    ...
    try {
      final url = Uri.parse('https://tym-whatsapp-backend.onrender.com/api/restaurants');
      ...
      final Map<String, dynamic> payload = {
        ...
        "primaryFlowType": _primaryFlow, // Now sends "ORDER_TYPE" correctly         
      ...
      if (response.statusCode == 200 || response.statusCode == 201) {
        ...
        String generatedId = "ID_NOT_FOUND";
        if (responseData.containsKey('restaurant') && responseData['restaurant'] != null) {
          generatedId = responseData['restaurant']['_id'] ?? "ID_NOT_FOUND";
        }

        _showSuccessDialog(generatedId);
      } else {
        ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text("Backend Error ${response.statusCode}: ${response.body}")));
      }
    } catch (e) {
        ...
        child: Padding(
          ...
          child: Column(
            ...
            children: [
              ...
              _buildContentCard(
                title: "Basic Details & Map", 
                subtitle: "Restaurant identity and physical location",
                  ...
                  children: [
                    ...
                    Row(
                      children: [
                        ...
                        Expanded(child: _buildInputField("Longitude", _lngController, isNumber: true)),
                      ],
                    )
                  ],
                )
              ...
              _buildContentCard(
                ...
                subtitle: "Fees, taxes, and service availability",
                icon: Icons.local_shipping_outlined, 
                content: Column(
                  children: [
                    Row(
                      children: [
                        Expanded(child: _buildInputField("GST Rate (%)", _gstRateController, isNumber: true)),
                        const SizedBox(width: 16),
                        Expanded(child: _buildInputField("Delivery Fee (₹)", _deliveryFeeController, isNumber: true)),
                        const SizedBox(width: 16),
                        Expanded(child: _buildInputField("Delivery Radius (KM)", _radiusController, isNumber: true)),
                      ],
                    ),
                    _buildDropdownField("Service Type", _selectedService, const [
                      DropdownMenuItem(value: "DELIVERY", child: Text("Delivery Only")),
                      DropdownMenuItem(value: "TAKEAWAY", child: Text("Takeaway Only")),
                      DropdownMenuItem(value: "BOTH", child: Text("Delivery & Takeaway")),
                    ], (v) => setState(() => _selectedService = v!)),
                    _buildDropdownField("Payment Availability", _selectedPayment, const [
                      DropdownMenuItem(value: "COD", child: Text("Cash on Delivery")),
                      DropdownMenuItem(value: "ONLINE", child: Text("Online Payments")),
              ...
              _buildContentCard(
                title: "Bot Ordering Flow", 
                subtitle: "Configure how the WhatsApp bot interacts with customers",
                icon: Icons.smart_toy_outlined, 
                content: Column(
                  children: [
                    // 🚀 CRITICAL FIX: The value is now "ORDER_TYPE"
                    _buildDropdownField("Primary Flow Type", _primaryFlow, const [
                    ...
                    ], (v) => setState(() => _primaryFlow = v!)),
                    
                    // 🚀 CRITICAL FIX: The conditional statement matches "ORDER_TYPE"
                    if (_primaryFlow == "ORDER_TYPE")
                      _buildDropdownField("Delivery Flow Type (Conditional)", _deliveryFlow, const [
                        DropdownMenuItem(value: "LOCATION_FIRST", child: Text("Location First Flow")),
                        DropdownMenuItem(value: "LOCATION_LAST", child: Text("Location Last Flow")),
                      ], (v) => setState(() => _deliveryFlow = v!)),
              ...
              _buildContentCard(
                ...
                subtitle: "Meta, Razorpay, and Google Sheets connections",
                ...
                content: Column(
                  ...
                  children: [
                    ...
                    _buildInputField("WABA ID *", _wabaIdController),
                    _buildInputField("Phone Number ID", _phoneIdController),
                    _buildInputField("WhatsApp Access Token *", _tokenController),
                    _buildInputField("Meta Catalog ID", _catalogIdController),
                    
                    const Divider(height: 40),
                    const Text("Razorpay Credentials", style: TextStyle(fontWeight: FontWeight.bold, color: Color(0xFF111827))),
                    const SizedBox(height: 12),
                    _buildInputField("Razorpay Key ID", _razorpayIdController),
                    _buildInputField("Razorpay Key Secret", _razorpaySecretController),
                    _buildInputField("Razorpay Webhook Secret", _razorpayWebhookController),

                    const Divider(height: 40),
                    const Text("Google Sheets (Exporting)", style: TextStyle(fontWeight: FontWeight.bold, color: Color(0xFF111827))),
                    const SizedBox(height: 12),
                    _buildInputField("Google Sheet URL", _sheetUrlController),
                    _buildInputField("Google Sheet ID", _sheetIdController),
                  ],
                )
              ),
